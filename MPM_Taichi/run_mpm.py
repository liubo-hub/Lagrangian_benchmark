...
def run_collision(i, inputs):
    ...
    # init MPM solver
    # combined_particles = ti.Vector.field(2, dtype=ti.f32, shape=200)
    ...
    particle_type_soil = np.full(n_soil_particles, 0)
    #gen obstacles num_particles
    if obstacles is not None:
        ...
        # Make particle type array  particle_type_obstacle:3
        # print(mpm.particle_info()["position"].shape)
        # print(mpm.particle_info()['material'])
    ...
    for frame in tqdm(range(nsteps)):
        ...
        if is_realtime_vis:
            if ndim == 3:
                ...
                gui.show()
            if ndim == 2:
                mapped_materials = np.mod(particles['material'], len(colors))
                gui.circles(particles['position'], radius=1.5, color=colors[mapped_materials])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_path', type=str, help="Input json file name")
