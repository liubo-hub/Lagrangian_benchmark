...
#     else:
#         if num == 1:
#             field[0] = start  # 当只有一个元素时，开始和结束应相同
...
class MPMSolver:
    ...
    @ti.kernel
    def seed_V2(self, new_material: ti.i32, color: ti.i32):
        new_particles = 800
        for i in range(self.n_particles[None], self.n_particles[None] + new_particles):
            # print("i:", i)
            self.material[i] = new_material
            x = gen_slope.combined_particles[i-self.n_particles[None]]
            # print("x:", x)
